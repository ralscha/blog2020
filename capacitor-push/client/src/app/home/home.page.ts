...
export class HomePage {
  errorMessage = signal('');
  allowPush = signal(false);
  allowPersonal = signal(false);
  ...
  constructor() {
    ...
    this.allowPush.set(pushFlag != null ? JSON.parse(pushFlag) : false);

    const personalFlag = localStorage.getItem('allowPersonal');
    this.allowPersonal.set(
      personalFlag != null ? JSON.parse(personalFlag) : false,
    );
  ...
  async register(): Promise<void> {
    ...
    this.http
      .post(`${environment.serverURL}/register`, formData)
      .pipe(timeout(10000))
      .subscribe({
        next: () =>
          localStorage.setItem(
            'allowPersonal',
            JSON.stringify(this.allowPersonal()),
          ),
  ...
  async unregister(): Promise<void> {
    ...
    formData.append('token', token);
    this.http
      .post(`${environment.serverURL}/unregister`, formData)
      .pipe(timeout(10000))
      .subscribe({
        next: () =>
          localStorage.setItem(
  ...
  handleNotification(data: { text: string; id: number }): void {
    ...
    this.items.update((items) =>
      [{ id: data.id, text: data.text }, ...items].slice(0, 5),
    );
  }
  ...
  async askFcmPermission(): Promise<boolean> {
    ...
    let status: 'prompt' | 'prompt-with-rationale' | 'granted' | 'denied' =
      checked.receive;
